...
function SectionTitle({ children }: PropsWithChildren) {
  return (
    <h3 className="mb-1.5 mt-5 text-sm font-semibold leading-relaxed text-foreground">
      {children}
    </h3>
  )
}
...
function OrderedList({ items }: { items: string[] }) {
  return (
    <ol className="ml-6 list-decimal text-sm leading-relaxed text-muted-foreground">
...
export function Content({ insight }: ContentProps) {
  ...
  return (
    <div className="lg:max-h-[23rem] lg:overflow-y-auto lg:pr-2 lg:[scrollbar-color:var(--border)_transparent]">
