...
export function buildAIPrompt(simulation: SimulationRecord) {
  const income = simulation.income ?? '0'
  const expenses = simulation.expenses ?? '0'
  const debts = simulation.debts ?? '0'
  const goalName = simulation.goalName ?? 'Meta'
  const goalAmount = simulation.goalAmount ?? '0'
  const goalDeadline = simulation.goalDeadline ?? '1'

  const monthlySavings = calcMonthlySavings(simulation)
  const monthlySavingsNeeded =
    parseCurrency(goalAmount) / Number.parseInt(goalDeadline, 10)
  const balanceAfterGoal = monthlySavings - monthlySavingsNeeded
...
- Saldo após reserva para a meta: ${balanceAfterGoal} reais
